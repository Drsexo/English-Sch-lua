...
gentab:add_text("Minimum resolution requirement: 1920X1080. To use the player function, please select a player in the yim player list and turn to the bottom of the player page") 
    ...
    if  vehgodr:is_enabled() then --控制远程载具无敌
        if loopa14 == 0 then
            if not PED.IS_PED_IN_ANY_VEHICLE(PLAYER.GET_PLAYER_PED(network.get_selected_player()),true) then
                ...
                gui.show_message("sch lua","Request control was successful")
                    --如果未被作弊者拦截,理论上应该Request control was successful了
